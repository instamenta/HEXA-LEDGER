 ...
 * @returns
 */
function getUsers(page = 1, limit = 5, filter?: string): Promise<PostGrpcModel[]> {
	return new Promise((resolve, reject) => {
		...
		const users: PostGrpcModel[] = [];
		const $stream = CLIENT.getUsers(m);
		$stream.on('data', (response: UserModel) => {
			users.push(PostGrpcModel.fromUserGRPCMessage(response));
		});
 ...
 * @returns
 */
function getAllUsers(page = 1, limit = 5): Promise<PostGrpcModel[]> {
	return new Promise((resolve, reject) => {
 ...
 * @returns
 */
function getUserById(id: string): Promise<PostGrpcModel> {
	return new Promise((resolve, reject) => {
		...
		CLIENT.getUserById(m, (err: GRPC.ServiceError, response: UserModel) => {
			err ? reject(err.message)
				: resolve(PostGrpcModel.fromUserGRPCMessage(response));
		});
	});
}
 ...
 * @returns
 */
function getUserFollowers(id: string, page = 1, limit = 5): Promise<PostGrpcModel[]> {
	return new Promise((resolve, reject) => {
		...
		const followers: PostGrpcModel[] = [];
		const $stream = CLIENT.getUserFollowers(m);
		$stream.on('data', (response: UserModel) => {
			followers.push(PostGrpcModel.fromUserGRPCMessage(response));
		});
		$stream.on('error', (err: GRPC.ServiceError) => {
			reject(err.message);
		});
 ...
 * @returns
 */
function getUserFollowing(id: string, page = 1, limit = 5): Promise<PostGrpcModel[]> {
	return new Promise((resolve, reject) => {
		...
		const following: PostGrpcModel[] = [];
		const $stream = CLIENT.getUserFollowing(m);
		$stream.on('data', (response: UserModel) => {
			following.push(PostGrpcModel.fromUserGRPCMessage(response));
		});
		$stream.on('error', (err: GRPC.ServiceError) => {
			reject(err.message);
		});
