 ...
 * @throws
 */
export function GET_USERS(call: ServerWritableStream<GetUsersRequest, IUserModel>): void {
 ...
 * @throws
 */
export function GET_ALL_USERS(call: ServerWritableStream<GetAllUsersRequest, IUserModel>): void {
   const limit: number = call.request.hasLimit() ? call.request.getLimit()!.getValue() : 5
      , page: number = call.request.hasPage() ? call.request.getPage()!.getValue() : 1
    ;
 ...
 * @throws
 */
export async function GET_USER_FOLLOWERS(call: ServerWritableStream<GetUserFollowersRequest, IUserModel>): Promise<void> {
 ...
 * @throws
 */
export async function GET_USER_FOLLOWING(call: ServerWritableStream<GetUserFollowingRequest, IUserModel>): Promise<void> {
...
export async function FOLLOW_USER(
   ...
   MongooseUserModel.collection.bulkWrite(
        ...
        }]).then((result: BulkWriteResult) => (result && result.ok)
...
export async function UNFOLLOW_USER(
   ...
   const currentUserId: ObjectId = Validator['CONVERT_TO_OBJECT_ID'](call.request.hasCurrentUserId()
      ? call.request.getCurrentUserId()!.getValue() : null)

      , userId: ObjectId = Validator['CONVERT_TO_OBJECT_ID'](call.request.hasId()
