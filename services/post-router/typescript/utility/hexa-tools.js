...
function processOn(CASES) {
    CASES.forEach((TYPE) => {
        process.on(TYPE, (error) => {
            try {
                console.error(`${exports.Dot.GET('SERVICE_NAME', 'Post-Router-Service')} - process.on ${TYPE}`);
                console.error(error);
            }
            catch {
                process.exit(1);
...
function processOnce(CASES) {
    CASES.forEach((TYPE) => {
        process.once(TYPE, (error) => {
            try {
                console.error(`${exports.Dot.GET('SERVICE_NAME', 'Post-Router-Service')} - process.on ${TYPE}`, error);
                process.exit(0);
            }
            finally {
