...
exports.RespondToZodError = exports.RespondGeneralPurpose = exports.RespondToError = exports.RespondMongoToError = exports.HandleMongoError = exports.ZodIssueCode = void 0;
const http_status_codes_1 = __importDefault(require("@instamenta/http-status-codes"));
const mongodb_1 = require("mongodb");
const zod_1 = require("zod");
const vlogger_1 = __importDefault(require("@instamenta/vlogger"));
const vlog = vlogger_1.default.getInstance().getVlogger('General Purpose');
...
function HandleMongoError(error) {
    if (error instanceof mongodb_1.MongoError) {
        ...
        vlog.error({ m: '🌐 MongoNetworkError:', e: error });
    }
}
...
function RespondToError(error, w) {
    vlog.error({ m: 'Server ran into error', f: 'RespondToError', e: error });
...
function RespondToZodError(error, res) {
    if (error instanceof zod_1.ZodError) {
        vlog.error({ e: error.message, m: '❌ Zod Validation Error:', f: 'RespondToZodError' });
        const errorResponse = {
            details: [],
        };
        error.errors.forEach((validationError) => {
            switch (validationError.code) {
                ...
                case exports.ZodIssueCode.not_multiple_of:
                    ...
                    break;
                case exports.ZodIssueCode.not_finite:
